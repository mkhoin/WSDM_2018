...
needs(tidyverse,scales,grid,gridExtra,RColorBrewer,corrplot,readr,
        data.table,tibble,tidyr,stringr,forcats,lubridate,ggforce,ggridges,h2o)

h2o.init(ip="localhost",min_mem_size='64g')
localH2O <<- h2o.init(ip =  "localhost", port = 54321, nthreads = -1, max_mem_size = "64g")
```


## Load data
```{r}
trans <- read_csv('C:/Users/Owner/Rstudio/data/transactions.csv')
trans_v2 <- read_csv('C:/Users/Owner/Rstudio/data/transactions_v2.csv')
trans <- bind_rows(trans,trans_v2)
rm(trans_v2);gc()

members_df <- read_csv('C:/Users/Owner/Rstudio/data/members_v3.csv')

train <- read_csv("C:/Users/Owner/Rstudio/data/train_scala_labeled.csv")

train_v2 <- read_csv("C:/Users/Owner/Rstudio/data/train_v2.csv")
train_v2$gap <- 0
train_v2$historyCutoff <- 20170331

train <- train %>% bind_rows(train_v2)
rm(train_v2);gc()
```
...
train <- read_csv("train_scala_labeled.csv")
train_v2 <- read_csv("../input/train_V2.csv")
...
members_df <- read_csv('../input/members_v3.csv')

date_list <- c(20160831,20160930,20161031,
               20161130,20161231,20170131,20170331)
```
...
table_201703$trans_date<-as.Date(table_201703$trans_date)+30*(7-i)
table_201703$exp_date<-as.Date(table_201703$exp_date)+30*(7-i)
...
```{r}
date_list <- c(201609,201610,201611,
...
total<-c()
for (i in 1:length(date_list)){
temp <- read_csv(paste0("jiseung_",
       as.numeric(date_list[i]),".csv"))
total <- total %>% bind_rows(temp)
}

write.csv(total,"table_train_df_201609_201702.csv",row.names=FALSE)
```

```{r}
test<-read_csv("jiseung_201704.csv")
write.csv(test,"table_test_df_201704.csv",row.names=FALSE)
```
